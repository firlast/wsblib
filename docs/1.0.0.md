...
while True:
    ...
    if processed_request:
        ...
        if process_type == 'route':
            response = processed_request.route.get_route_response(request_data)
        else:
            response = processed_request.route.get_callback_response(request_data)
...
3. Checking request result: this condition serves to ignore requests from clients that have not sent any HTTP messages, processing only true requests. Inside the condition, we get two pieces of data from the ***processed request***: the request data in `processed_request.request` (like cookies, headers, method, path, etc) and then we get the response type (in `processed_request.type`), to know if it is an error or a response generated by the requested route.
...
4. Getting request response: this condition gets the response to the request according to the status of the response, whether it was a response generated by the route or an error (404 Not Found or 405 Method Not Allowed, for example). If the response was generated by the route, we use the `processed_request.route.get_route_response` method, passing the `request_data` variable as an argument. Otherwise, if the response is an error, we use the `processed_request.route.get_callback_response` method. Both methods returned a `Response` object, from the [HTTPPyParser](https://github.com/jaedsonpys/http_pyparser) library, containing all the response data
...
5. Make HTTP response: We use the [HTTPPyParser](https://github.com/jaedsonpys/http_pyparser) library to generate the response in raw HTTP. Just pass a `Response` object (also from the library) to the `http_pyparser.response.make_response` method, which returns a string.

```python
    http = http_pyparser.response.make_response(response)
```

6. Sending response to client: Finally, we send the HTTP message stored in the `http` variable using the `send_message` method of the `Client` class. After that, we use the `destroy()` method to close the connection with the client.
...
After these **06 steps**, the request was processed and the response was sent to the client.